...
def convert_to_gan_reading_format_save(input_dir, output_dir, target_size, bucket_size):
    ...
    # (1) get list of all files in directory tree res/data/iamDB/words/
    listOfFiles = list()
    for (dirpath, dirnames, filenames) in os.walk(input_dir): #dirpath = current path, dirnames = directory in current path, filename = files in current path
        listOfFiles += [os.path.join(dirpath, file) for file in filenames]

    listOfFiles = [name for name in os.listdir(input_dir) if \
