...
def get_parse():
    ...
    args.add_argument(
        '--full-gen',
        help='Fully generating from Viet74K',
        action='store_true'
    )
...
texts = []
invalid_chars = [',', '.', '?', '[', ']', '!', '#', '$', ':', ';']
with open(text_paths, 'r', encoding='utf-8') as f:
...
def write_annot(image_name, label, img_id):
    if img_id == 0:
        with open(save_annot_path, 'w', encoding='utf-8') as f:
            f.write(f"{image_name} {label}\n")
    else:
        with open(save_annot_path, 'a',encoding='utf-8') as f:
            f.write(f"{image_name} {label}\n")
...
def show_or_save(text: str, n_samples: int, save=False, show=True):
    ...
    for i in range(predictions.shape[0]):
        ...
        if show:
            ...
            plt.show()
        if save:
            image_path = os.path.join(save_image_dir, f"gen_image_{img_id}.jpg")
...
def main():
    ...
    if not args.full_gen:
        texts = texts[:num_uniqnues]

    print(f"[INFO] Generating {len(texts)} unique texts")
    print(f"[INFO] Generating {args.variation} variants for each image")
